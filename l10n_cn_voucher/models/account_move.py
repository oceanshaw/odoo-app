...
class AccountMove(models.Model):
    ...
    period_id = fields.Many2one(string='会计期间', states=READONLY_STATE)
    ...
    def _add_field(self, name, field):
        ...
        """
        res = super(AccountMove, self)._add_field(name, field)
        ...
        for move in self:
            if move.state != 'post':
                if move.approval_uid.id == False:
                    ...
                    move.write({'approval_uid': self.env.uid, 'state': 'validate'})
                else:
                    move.write({'approval_uid': False, 'state': 'draft'})
